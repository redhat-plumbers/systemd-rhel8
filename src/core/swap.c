...
static int swap_add_device_dependencies(Swap *s) {
        ...
        if (is_device_path(p->what)) {
                r = unit_add_node_dependency(UNIT(s), p->what, MANAGER_IS_SYSTEM(UNIT(s)->manager), UNIT_REQUIRES, mask);
                if (r < 0)
                        return r;
        ...
        return unit_add_dependency_by_name(UNIT(s), UNIT_AFTER, SPECIAL_REMOUNT_FS_SERVICE, NULL, true, mask);
}

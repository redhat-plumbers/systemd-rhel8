...
void unit_notify(Unit *u, UnitActiveState os, UnitActiveState ns, UnitNotifyFlags flags) {
        ...
        if (!MANAGER_IS_RELOADING(m)) {
                ...
                /* If this state change happened without being requested by a job, then let's retroactively start
                 * or stop dependencies. We skip that step when deserializing, since we don't want to create any
